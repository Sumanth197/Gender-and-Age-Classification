...
tf.app.flags.DEFINE_integer('class_num', 8, "Num of classes")
...
tf.app.flags.DEFINE_integer('epoch_num', 1, "Num of epochs.")
tf.app.flags.DEFINE_string('train_data_dir', './age_data/train/', "Train data dir while images in")
tf.app.flags.DEFINE_string('val_data_dir', './age_data/validation/', "Validation data dir while images in")
tf.app.flags.DEFINE_string('model_path', './age_model/', "Path the model in")
...
tf.app.flags.DEFINE_string('test_pic', 'test.jpg', "Test picture.")
tf.app.flags.DEFINE_string('char_dict', './char_dict.bin', "Char dict.")
tf.app.flags.DEFINE_string('log_dir', './log_dir', "Tf summary log.")
tf.app.flags.DEFINE_integer('run_type', 0, "0 for training;1 for testing all model;"
...
class DataReader:
    ...
    def preprocess(self, image):
        """
        data augmentation: crop, flip, bright, contrast, scale, rotate.
        :param image:
        :return:
        """
...
def inference(image):
    ...
    # restore model
    model_name = os.path.join(FLAGS.model_path, 'model.ckpt-0')
    saver = tf.train.import_meta_graph(model_name + '.meta')
    saver.restore(sess, model_name)
